...
# ページ設定（ブラウザータブのタイトルを設定）
st.set_page_config(page_title="アルコール摂取量計算機", page_icon="🍺", layout="wide")
...
def calculate_alcohol_volume(alcohol_percentage, target_alcohol_grams):
    ...
    alcohol_volume = (target_alcohol_grams * 100) / (
        alcohol_percentage * alcohol_density
    )
...
def main():
    ...
    st.write(
        "目標とするアルコール量と飲み物のアルコール度数から、摂取すべき量を計算します。"
    )
    ...
    if st.button("計算実行", type="primary"):
        try:
            ...
            with col2:
                st.metric(label="アルコール度数", value=f"{alcohol_percentage}%")

            with col3:
                st.metric(label="目標アルコール量", value=f"{target_alcohol_grams} g")
